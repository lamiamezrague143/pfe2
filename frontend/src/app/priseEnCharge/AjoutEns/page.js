...
export default function AddTeacherForm() {
  ...
  const addAyantDroit = () => {
    ...
    if (newAyantDroit.lien === "Enfant" && newAyantDroit.sexe === "Masculin" && age >= 21) {
      alert("Erreur : Un enfant de sexe masculin doit avoir moins de 21 ans pour être pris en charge.");
      return;
    }
  ...
  // ── Soumission ─────────────────────────────────────────────────────────────
  // ⚠️ handleSubmit utilise FormData (multipart) pour envoyer des fichiers.
  // apiFetch force "Content-Type: application/json", donc on appelle fetch
  // directement ici en injectant manuellement le token.
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(url, {
        ...
        body: dataToSend, // multipart/form-data, pas de Content-Type manuel
      });
    ...
    <div className="max-w-6xl mx-auto p-4 md:p-10">
      ...
      <div className="flex gap-2 mb-8 bg-gray-100 p-1 rounded-2xl w-fit">
        <button
          ...
          className={`flex items-center gap-2 py-3 px-6 rounded-xl font-bold transition-all ${vueActive === "formulaire" ? "bg-white text-green-800 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
        >
          <UserPlus size={18} /> {isEditing ? "Modifier" : "Ajouter"} Enseignant
        </button>
        <button
          onClick={() => setVueActive("liste")}
          className={`flex items-center gap-2 py-3 px-6 rounded-xl font-bold transition-all ${vueActive === "liste" ? "bg-white text-green-800 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
        >
      ...
      {vueActive === "formulaire" ? (
        <div className="bg-white rounded-3xl shadow-xl p-10 border border-gray-100 animate-in fade-in duration-500">
            ...
            <div className="relative group" onClick={() => fileInputRef.current.click()}>
              <div className="w-28 h-28 rounded-2xl border-4 border-dashed border-gray-200 overflow-hidden cursor-pointer flex items-center justify-center bg-gray-50 hover:border-green-700 transition-all">
                {preview
                  ...
                  : <div className="text-center text-gray-400"><Camera size={24} className="mx-auto" /><span className="text-[10px] font-bold uppercase">Photo</span></div>
                }
              </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="bg-gray-50/50 p-8 rounded-3xl border border-gray-100">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="text-slate-800 font-black flex items-center gap-2 text-lg uppercase tracking-tight">
                    <PlusCircle size={22} className="text-green-800" /> Membres de la Famille
                  </h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mb-6">
                <div className="space-y-1">
                  ...
                  <input placeholder="Nom de famille" value={newAyantDroit.nom} onChange={(e) => setNewAyantDroit({ ...newAyantDroit, nom: e.target.value })} className="w-full p-3.5 bg-white border-none rounded-2xl shadow-sm focus:ring-2 focus:ring-green-100 outline-none transition-all" />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Prénom</label>
                  <input placeholder="Prénom" value={newAyantDroit.prenom} onChange={(e) => setNewAyantDroit({ ...newAyantDroit, prenom: e.target.value })} className="w-full p-3.5 bg-white border-none rounded-2xl shadow-sm focus:ring-2 focus:ring-green-100 outline-none transition-all" />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Date de Naissance</label>
                  <input type="date" value={newAyantDroit.dateNaissance} onChange={(e) => setNewAyantDroit({ ...newAyantDroit, dateNaissance: e.target.value })} className="w-full p-3.5 bg-white border-none rounded-2xl shadow-sm focus:ring-2 focus:ring-green-100 outline-none transition-all text-gray-600" />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Lien de parenté</label>
                  <select value={newAyantDroit.lien} onChange={(e) => setNewAyantDroit({ ...newAyantDroit, lien: e.target.value })} className="w-full p-3.5 bg-white border-none rounded-2xl shadow-sm focus:ring-2 focus:ring-green-100 outline-none transition-all text-gray-600">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    <select value={newAyantDroit.sexe} onChange={(e) => setNewAyantDroit({ ...newAyantDroit, sexe: e.target.value })} className="w-full p-3.5 bg-white border-none rounded-2xl shadow-sm focus:ring-2 focus:ring-green-100 outline-none transition-all text-gray-600">
                  ...
                  <div className="space-y-1">
                    ...
                    <select value={newAyantDroit.situationMatrimoniale} onChange={(e) => setNewAyantDroit({ ...newAyantDroit, situationMatrimoniale: e.target.value })} className="w-full p-3.5 bg-white border-none rounded-2xl shadow-sm focus:ring-2 focus:ring-green-100 outline-none transition-all text-gray-600">
                ...
                <div className="flex items-end gap-3">
                  ...
                  <button type="button" onClick={addAyantDroit} className="h-[52px] px-6 bg-green-900 text-white rounded-2xl font-black hover:bg-green-800 transition-all shadow-md active:scale-95">
                    AJOUTER
                  </button>
                </div>
              </div>
                ...
                {ayantDroits.map((ad, i) => (
                  <div key={i} className="flex items-center gap-3 bg-white pl-2 pr-4 py-2 rounded-2xl border border-gray-100 shadow-sm group animate-in zoom-in-95 duration-200">
                    <div className="w-10 h-10 rounded-xl overflow-hidden bg-gray-50 border border-gray-100">
                      <img src={ad.photo ? (ad.photo instanceof File ? URL.createObjectURL(ad.photo) : ad.photo) : `https://ui-avatars.com/api/?name=${ad.nom}+${ad.prenom}&background=random`} className="w-full h-full object-cover" />
                    </div>
                    <div>
                    ...
                    <button type="button" onClick={() => removeAyantDroit(i)} className="ml-2 p-1.5 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all">
                      <Trash2 size={16} />
                    </button>
                  </div>
                ))}
            ...
            {status.message && (
              <p className={`text-center font-bold ${status.type === "success" ? "text-green-600" : "text-red-600"}`}>
                {status.message}
              </p>
            )}
          </form>
        ...
        <section className="bg-white rounded-3xl shadow-xl p-8 animate-in fade-in duration-500 border border-gray-100">
          <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
            ...
            <div className="flex gap-3 w-full md:w-auto">
              <button onClick={chargerUsers} className="p-3 bg-green-50 text-green-700 rounded-xl hover:bg-green-100 transition-colors border border-green-100" title="Actualiser la liste">
                <RotateCcw size={20} />
              </button>
                    ...
                    <React.Fragment key={user.id}>
                      <tr className="bg-gray-50 hover:bg-green-50 transition-all rounded-2xl">
                        ...
                        <td className="px-6 py-4 rounded-r-2xl text-right flex items-center justify-end gap-2">
                          <button onClick={() => startEdit(user)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg" title="Modifier"><Edit3 size={18} /></button>
                          <button onClick={() => deleteUser(user.id)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg" title="Supprimer"><Trash2 size={18} /></button>
