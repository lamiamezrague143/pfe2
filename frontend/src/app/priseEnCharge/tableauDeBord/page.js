...
export default function TableauDeBord() {
  ...
  const [cliniques, setCliniques]   = useState([]); // ✅ AJOUT
  ...
  const fetchData = async () => {
    try {
      ...
      // ✅ AJOUT : fetch des cliniques pour mapper ID → nom
      const resCliniques = await fetch("http://localhost:5001/api/clinics/all");
      if (resCliniques.ok) {
      ...
      if (resHis.ok) {
        ...
        setHistory(
          allData.filter(item => {
            ...
            const matchYear  = selectedYear  ? date.getFullYear()    === Number(selectedYear)  : true;
            const matchMonth = selectedMonth ? date.getMonth() + 1   === Number(selectedMonth) : true;
  ...
  // ✅ CORRIGÉ : mapping sfEtablissement (ID) → nom de la clinique
  const statsClinique = history.reduce((acc, curr) => {
  ...
  Object.entries(avenantsByClient).forEach(([clientKey, data]) => {
    data.avenants.forEach(av => {
      ...
      if (!target[clientKey]) {
        target[clientKey] = { nom: clientKey, prenom: "", prises: [], prisesAnnulees: [], avenants: [], totalConsomme: 0 };
      }
          ...
          <table className="w-full text-left min-w-[1000px]">
            <thead>
              <tr className="bg-blue-900 text-white text-[10px] uppercase tracking-widest">
                ...
                <th className="p-4 text-center border-l border-blue-800">Nb Prises</th>
                ...
                {hasAnyAnnulee && annuleeColumns.map(num => (
                  <th key={`xh-${num}`} className="p-4 border-l border-red-400 text-center bg-red-900/30 text-red-200">Prise Annulée {num}</th>
                ))}
                ...
                filtered.map((client, idx) => {
                  ...
                  return (
                    <tr key={idx} className={`border-b border-gray-50 ${status.row} transition-colors hover:bg-blue-50/30`}>
                      ...
                      <td className="p-4 text-center font-bold text-gray-700 border-l">{client.prises.length}</td>
                      ...
                      {hasAnyAnnulee && annuleeColumns.map((_, i) => {
                        ...
                        return (
                          <td key={`xc-${i}`} className={`p-4 text-center border-l border-red-100 min-w-[130px] ${ann ? 'bg-red-50' : ''}`}>
                            {ann ? (
                              <div className="flex flex-col items-center">
                                <span className="text-[9px] font-bold text-red-300 uppercase leading-tight">{ann.prestation}</span>
                                <span className="text-[11px] font-black text-red-500 line-through">{ann.montant.toLocaleString()} DA</span>
                                <span className="text-[8px] text-red-400 font-black">ANNULÉ</span>
                              </div>
                            ) : (
                              <span className="text-gray-300 text-[10px]">—</span>
                            )}
...
return (
    <div className="min-h-screen bg-slate-100 p-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* ── HEADER ── */}
        <header className="mb-10 bg-green-800 rounded-2xl p-6 shadow-xl flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-black text-white uppercase tracking-tight">Tableau de Bord</h1>
            <p className="text-blue-300 font-bold text-sm mt-1">Exercice Annuel {new Date().getFullYear()}</p>
          </div>

          {/* COMPTEUR + FILTRE DATE */}
          <div className="flex flex-col items-center gap-2">
            <div className="bg-white/10 border border-white/20 text-white px-6 py-3 rounded-xl text-center min-w-[180px]">
              <p className="text-[10px] uppercase font-bold tracking-widest text-blue-200">Total Prises en Charge</p>
              <p className="text-4xl font-black mt-1">
                {filterDate
                  ? history.filter(item => new Date(item.createdAt).toISOString().split("T")[0] === filterDate).length
                  : history.length
                }
              </p>
              {filterDate && (
                <p className="text-[10px] text-blue-200 mt-0.5">
                  le {new Date(filterDate).toLocaleDateString('fr-FR', { day: '2-digit', month: 'long', year: 'numeric' })}
                </p>
              )}
            </div>
            <div className="flex items-center gap-2">
              <input
                ...
                className="p-2 bg-white/10 border border-white/20 rounded-lg text-sm font-bold outline-none text-white placeholder-blue-200"
              />
              {filterDate && (
                <button onClick={() => setFilterDate("")} className="text-[11px] text-blue-200 hover:text-red-300 font-bold transition">
                  ✕ Tout
                </button>
              )}
            </div>
          ...
          <div className="text-right text-[10px] font-bold text-blue-300 uppercase tracking-widest leading-relaxed">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
          {[
            { label: "Prises Actives",  val: history.filter(h => !h.annule).length,  color: "border-blue-500",   text: "text-blue-700",   bg: "bg-blue-50"   },
            { label: "Annulées",        val: history.filter(h => h.annule).length,   color: "border-red-500",    text: "text-red-700",    bg: "bg-red-50"    },
            { label: "Soins Dentaires", val: history.filter(h => (h.prestation || h.type_prestation || "").toUpperCase().includes("DENT")).length, color: "border-teal-500", text: "text-teal-700", bg: "bg-teal-50" },
            { label: "Ophtalmo",        val: history.filter(h => (h.prestation || h.type_prestation || "").toUpperCase().includes("OPHTA")).length, color: "border-purple-500", text: "text-purple-700", bg: "bg-purple-50" },
          ].map(({ label, val, color, text, bg }) => (
            <div key={label} className={`${bg} border-l-4 ${color} rounded-xl p-5 shadow-sm`}>
        ...
        <div className="flex flex-col md:flex-row gap-6 mb-10">
          ...
          <PieChart title="Répartition par Grades"      data={statsGrades}      colors={["#0f172a", "#3b82f6", "#94a3b8", "#64748b"]} />
        </div>

        {/* ── TOP 5 PRESTATIONS ── */}
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 mb-6">
          <h2 className="text-sm font-black text-slate-500 mb-6 uppercase tracking-widest border-b pb-4">
          ...
          <div className="h-[350px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={dataClinique} layout="vertical" margin={{ left: 20, right: 30 }}>
                ...
                <YAxis type="category" dataKey="name" axisLine={false} tickLine={false} tick={{fill: '#1e3a5f', fontSize: 10, fontWeight: 800}} width={160} />
                <Tooltip cursor={{fill: '#f8fafc'}} contentStyle={{borderRadius: '12px', border: 'none', boxShadow: '0 4px 20px rgba(0,0,0,0.08)'}} />
                <Bar dataKey="total" radius={[0, 8, 8, 0]} barSize={28}>
                  {dataClinique.map((entry, index) => (
                    <Cell key={`cell-clinic-${index}`} fill={['#1e3a8a','#1d4ed8','#3b82f6','#60a5fa','#93c5fd','#bfdbfe'][index % 6]} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
        ...
        {/* ── PLAFONDS + FILTRE ── */}
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 mb-10 grid grid-cols-1 md:grid-cols-4 gap-6">
          <div>
            <label className="block text-[10px] font-black text-gray-400 uppercase mb-2 tracking-widest">Plafond Général (DA)</label>
            <input type="number" value={plafondGeneral} onChange={(e) => updatePlafondDB('plafond_general', e.target.value)}
              className="w-full p-3 bg-blue-50 border border-blue-100 rounded-xl font-bold text-blue-900 outline-none focus:ring-2 focus:ring-blue-300" />
          </div>
          <div>
            <label className="block text-[10px] font-black text-gray-400 uppercase mb-2 tracking-widest">Plafond Dentaire (DA)</label>
            <input type="number" value={plafondDentaire} onChange={(e) => updatePlafondDB('plafond_dentaire', e.target.value)}
              className="w-full p-3 bg-teal-50 border border-teal-100 rounded-xl font-bold text-teal-700 outline-none focus:ring-2 focus:ring-teal-300" />
          </div>
          <div>
            <label className="block text-[10px] font-black text-gray-400 uppercase mb-2 tracking-widest">Plafond Ophtalmique (DA)</label>
            <input type="number" value={plafondOphta} onChange={(e) => updatePlafondDB('plafond_ophta', e.target.value)}
              className="w-full p-3 bg-purple-50 border border-purple-100 rounded-xl font-bold text-purple-700 outline-none focus:ring-2 focus:ring-purple-300" />
          </div>
          <div>
            <label className="block text-[10px] font-black text-gray-400 uppercase mb-2 tracking-widest">Rechercher un bénéficiaire</label>
            <input type="text" placeholder="Chercher un nom..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full p-3 border border-gray-200 rounded-xl shadow-sm outline-none font-bold focus:ring-2 focus:ring-gray-300" />
          </div>
        </div>
