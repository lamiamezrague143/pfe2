...
export default function GestionDossiersSecretariat() {
  ...
  const chargerDonnees = useCallback(async () => {
    try {
      ...
      const [resUsers, resDossiers, resPrestations] = await Promise.all([
        axios.get(`${API_BASE}/users/all`, { headers: { Authorization: `Bearer ${token}` } }),
        axios.get(`${API_BASE}/dossiers/liste-generale`, { headers: { Authorization: `Bearer ${token}` } }),
        axios.get(`${API_BASE}/prestations/all`, { headers: { Authorization: `Bearer ${token}` } })
      ]);
  ...
  const getSpecificField = () => {
    ...
    if (typeLower.includes("retraite")) return { label: "Date de départ à la retraite", value: dateDepartRetraite, setValue: setDateDepartRetraite, type: "date" };
    if (typeLower.includes("frais funéraire") || typeLower.includes("décès") || typeLower.includes("dece")) return { label: "Nom du défunt", value: nomDefunt, setValue: setNomDefunt, type: "text", placeholder: "Nom complet du défunt" };
    if (typeLower.includes("naissance")) return { label: "Nom de l'enfant", value: nomEnfant, setValue: setNomEnfant, type: "text", placeholder: "Nom complet de l'enfant" };
    if (typeLower.includes("circoncision")) return { label: "Nom de l'enfant", value: nomEnfant, setValue: setNomEnfant, type: "text", placeholder: "Nom complet de l'enfant" };
    if (typeLower.includes("mariage")) return { label: "Date de mariage", value: dateMariage, setValue: setDateMariage, type: "date" };
    return null;
  };
  ...
  const ajouterPrestation = async () => {
    ...
    try {
      ...
      await axios.post(`${API_BASE}/prestations/ajouter`, { titre: newPrestationTitre.trim(), pieces: piecesArray });
      ...
      <header className="relative px-8 pt-12 pb-8 overflow-hidden bg-white">
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          ...
          <h2 className="text-4xl md:text-5xl font-black text-slate-900 mb-4 tracking-tight">
            Prestation <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-teal-600">SG/COS</span>
          </h2>
          <p className="text-slate-500 text-base max-w-2xl leading-relaxed">Gérez les dossiers des prestations des bénéficiaires.</p>
        </div>
      </header>
            ...
            <div className="bg-white rounded-2xl shadow-xl shadow-slate-100 border border-slate-100 overflow-hidden">
              <div className="px-6 py-5 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white">
                <div className="flex justify-between items-center">
                  ...
                  <button onClick={() => setShowModalPrestation(true)} className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 text-white rounded-xl text-xs font-bold hover:shadow-lg transition-all">
              ...
              <div className="p-6">
                <div className="relative mb-6 max-w-md">
                  ...
                  <input value={recherchePrestation} onChange={(e) => setRecherchePrestation(e.target.value)} placeholder="Rechercher une prestation..." className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500" />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {blocs.filter(b => (b.titre || "").toLowerCase().includes(recherchePrestation.toLowerCase())).map(b => (
                    <div key={b.id} className="group relative">
                      <button onClick={() => ouvrirFormulaire(b)} className="w-full flex items-center gap-3 p-4 rounded-xl border border-slate-200 bg-white hover:border-emerald-300 hover:bg-emerald-50/50 transition-all text-left">
                      ...
                      <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={(e) => { e.stopPropagation(); setEditPrestation(b); setEditTitre(b.titre); setEditPieces((b.pieces || []).map((nom, i) => ({ id: i + '_' + nom, nom, editing: false }))); setShowModalEdit(true); }} className="p-1.5 bg-white border border-slate-200 rounded-lg text-slate-400 hover:text-blue-600 hover:border-blue-200">
                          <PencilIcon className="w-3 h-3" />
                        </button>
                        <button onClick={(e) => { e.stopPropagation(); supprimerPrestation(b.id, b.titre); }} className="p-1.5 bg-white border border-slate-200 rounded-lg text-slate-400 hover:text-red-600 hover:border-red-200">
                          <TrashIcon className="w-3 h-3" />
                        </button>
                      </div>
                    </div>
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <div className="relative">
                        <MagnifyingGlassIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                        <input className="w-full pl-9 pr-3 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none font-medium text-sm focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500" placeholder="Nom du bénéficiaire..." value={nomRecherche}
                          onChange={(e) => { setNomRecherche(e.target.value); if (e.target.value.length > 1) { setSuggestions(utilisateurs.filter(u => (u.nomComplet || "").toLowerCase().includes(e.target.value.toLowerCase()))); } else setSuggestions([]); }} />
                        {suggestions.length > 0 && (
                          <div className="absolute z-30 mt-1 w-full bg-white shadow-xl rounded-xl border border-slate-100 overflow-hidden">
                            {suggestions.map(u => (
                              <div key={u.id} onClick={() => { setBeneficiaireSelectionne(u); setNomRecherche(`${u.nomComplet} ${u.prenomComplet}`); setSuggestions([]); }} className="p-3 hover:bg-emerald-50 cursor-pointer font-medium text-sm border-b last:border-0">
                      ...
                      <div className="bg-emerald-50/50 p-5 rounded-xl border border-emerald-100">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div className="col-span-2">
                            ...
                            <p className="font-bold text-emerald-700 text-sm bg-emerald-100 inline-block px-3 py-1 rounded-full">{beneficiaireSelectionne.departement || beneficiaireSelectionne.faculte || "Non spécifié"}</p>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    {specificField && specificField.type === "text" && (
                      <div className="bg-blue-50 p-5 rounded-xl border border-blue-100">
                        ...
                        <input type="text" placeholder={specificField.placeholder} className="w-full p-3 bg-white border border-blue-200 rounded-xl outline-none font-medium text-sm" value={specificField.value} onChange={(e) => specificField.setValue(e.target.value)} />
                      </div>
                    )}
                    ...
                    {specificField && specificField.type === "date" && (
                      <div className="bg-blue-50 p-5 rounded-xl border border-blue-100">
                        <label className="block text-xs font-black text-blue-700 uppercase tracking-wider mb-2">{specificField.label}</label>
                        <input type="date" className="w-full p-3 bg-white border border-blue-200 rounded-xl outline-none font-medium text-sm" value={specificField.value} onChange={(e) => specificField.setValue(e.target.value)} />
                      </div>
                    )}
                    ...
                    {isMontantRequired && (
                      <div className="bg-amber-50 p-5 rounded-xl border-2 border-amber-100">
                        ...
                        <input type="number" step="0.01" placeholder="0.00" className="w-full p-3 bg-white border-2 border-amber-200 rounded-xl outline-none font-black text-lg text-amber-700" value={montantAvenant} onChange={(e) => setMontantAvenant(e.target.value)} />
                      </div>
                    )}
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        {['ATS', 'Enseignant', 'Retraité'].map(f => (
                          <button key={f} onClick={() => setFonctionChoisie(f)} className={`flex-1 py-3 rounded-xl text-xs font-black border-2 transition-all ${fonctionChoisie === f ? "bg-emerald-600 border-emerald-600 text-white" : "bg-white border-slate-200 text-slate-400 hover:border-emerald-300"}`}>{f}</button>
                        ))}
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="space-y-3 mb-6">
                      {listePieces.map((p, i) => (
                        <div key={i} className={`flex items-center gap-3 p-3 rounded-xl bg-white border-2 transition-all ${p.cochee ? 'border-emerald-500 shadow-sm' : 'border-transparent'}`}>
                          <div onClick={() => { const copy = [...listePieces]; copy[i].cochee = !copy[i].cochee; setListePieces(copy); }} className={`w-5 h-5 rounded-md border-2 flex items-center justify-center cursor-pointer ${p.cochee ? 'bg-emerald-600 border-emerald-600 text-white' : 'border-slate-300'}`}>
                      ...
                      {listePieces.length === 0 && <div className="text-center py-8 text-slate-400 text-sm">Aucun document requis pour cette prestation</div>}
                    </div>
                    <button onClick={handleSubmit} disabled={!beneficiaireSelectionne || !fonctionChoisie || !toutesLesPiecesCochees || isSubmitting} className="w-full py-4 bg-gradient-to-r from-emerald-600 to-teal-600 text-white rounded-xl font-black text-sm shadow-lg disabled:opacity-30 transition-all hover:shadow-xl">
          ...
          <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                ...
                <PiecesManager pieces={newPrestationPieces} onAdd={ajouterPieceNouveauForm} onDelete={supprimerPieceNouveauForm} onStartEdit={startEditPieceNouveauForm} onSaveEdit={saveEditPieceNouveauForm} onCancelEdit={cancelEditPieceNouveauForm} onChangeTempNom={changeTempNomNouveauForm} inputValue={newPieceInput} onInputChange={setNewPieceInput} placeholder="ex: Demande manuscrite, Ordonnance..." />
              </div>
            </div>
          ...
          <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                ...
                <PiecesManager pieces={editPieces} onAdd={ajouterPieceEdit} onDelete={supprimerPieceEdit} onStartEdit={startEditPieceEdit} onSaveEdit={saveEditPieceEdit} onCancelEdit={cancelEditPieceEdit} onChangeTempNom={changeTempNomEdit} inputValue={editPieceInput} onInputChange={setEditPieceInput} placeholder="Ajouter un document..." />
              </div>
            </div>
            <div className="px-6 pb-6 pt-4 border-t border-slate-100 flex gap-3">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="mt-4"><p className="text-[10px] font-black text-slate-400 uppercase mb-1">Né(e) le</p><p className="font-bold text-slate-800">{accusereception.dateN} à {accusereception.lieuN}</p></div>
                  <div className="mt-4"><p className="text-[10px] font-black text-slate-400 uppercase mb-1">Faculté</p><p className="font-bold text-emerald-700">{accusereception.faculte}</p></div>
                  {accusereception.montantAffichage > 0 && <div className="mt-4 bg-amber-50 p-3 rounded-xl"><p className="text-[9px] font-black text-amber-600 uppercase">Montant Avenant</p><p className="font-black text-amber-800">{accusereception.montantAffichage.toLocaleString('fr-FR')} DA</p></div>}
                  {accusereception.nomDefunt && <div className="mt-4 bg-gray-50 p-3 rounded-xl"><p className="text-[9px] font-black text-gray-600 uppercase">Défunt</p><p className="font-black text-gray-800">{accusereception.nomDefunt}</p></div>}
                  {accusereception.nomEnfant && <div className="mt-4 bg-pink-50 p-3 rounded-xl"><p className="text-[9px] font-black text-pink-600 uppercase">Enfant</p><p className="font-black text-pink-800">{accusereception.nomEnfant}</p></div>}
                </div>
  ...
  function ajouterPieceNouveauForm() { const val = newPieceInput.trim(); if (!val) return; setNewPrestationPieces(prev => [...prev, { id: Date.now(), nom: val, editing: false }]); setNewPieceInput(''); }
  ...
  function ajouterPieceEdit() { const val = editPieceInput.trim(); if (!val) return; setEditPieces(prev => [...prev, { id: Date.now(), nom: val, editing: false }]); setEditPieceInput(''); }
